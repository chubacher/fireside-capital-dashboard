...
import { formatCurrency, getRaw,getNextDate } from './utility.js';
...
function renderNetWorthChart() {
  ...
  const theme = getThemeColors(); // Ensure this function is defined
...
function generateMonthlyCashFlowChart() {
  ...
  const theme = getThemeColors(); // Use theme colors
...
function getThemeColors() {
  ...
  return {
    fill: isDark ? 'rgba(0, 123, 255, 0.2)' : 'rgba(0, 123, 255, 0.1)',
    text: isDark ? '#eaeaea' : '#333',
    grid: isDark ? '#444' : '#ccc'
  };
}
  
export function dedupeSnapshotsByDate(snapshots) {
